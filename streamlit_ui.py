...
"""
                    ...
                    # Create ZIP with all files (image + SEO JSON + SEO TXT)
                    st.divider()
...
FILES INCLUDED:
--------------
1. {design_id}.png - The generated design image
2. {design_id}_seo.json - SEO metadata in JSON format
3. {design_id}_seo.txt - SEO metadata in text format
...
"""
                    ...
                    st.download_button(
                        ...
                        data=zip_data,
                        file_name=f"{design_id}_complete.zip",
                        mime="application/zip",
                        use_container_width=True,
                        key=f"download_complete_zip_{design_id}"
                    )
        ...
        if st.session_state.bulk_combinations:
            if st.button("🚀 Generate Batch", type="primary", use_container_width=True, 
                    ...
                    try:
                        ...
                        try:
                            ...
                            if get_api_key("ANTHROPIC_API_KEY") and ANTHROPIC_AVAILABLE:
                                try:
                                    seo_data = SEOGenerator.generate_seo_with_anthropic(
                                        output_path, combo['style'], combo['niche'], combo['text']
                                    )
                                except:
                                    pass
                            
                            # Try OpenAI if Vision failed
                            if not seo_data and get_api_key("OPENAI_API_KEY"):
                                try:
                                    seo_data = SEOGenerator.generate_seo_with_openai(
                                        combo['style'], combo['niche'], prompt, combo['text']
                                    )
                                except:
                                    pass
                            ...
                            if not seo_data:
                                seo_data = SEOGenerator.generate_seo_fallback(
                                    combo['style'], combo['niche'], combo['text']
                                )
                        except:
                            # Final fallback
                            seo_data = SEOGenerator.generate_seo_fallback(
                ...
                if successful_results:
                    # Create ZIP immediately (not inside button)
                    import zipfile
                    
                    zip_buffer = io.BytesIO()
                    
                    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                        # Add all images and their SEO files
                        for result in successful_results:
                            ...
                            # Add SEO JSON
                            if 'seo' in result and result['seo']:
                                seo_json = json.dumps(result['seo'], indent=2)
                                zip_file.writestr(f"{result['id']}_seo.json", seo_json.encode('utf-8'))
                                
                                # Add SEO TXT
                                seo_text = f"""Title: {result['seo'].get('title', '')}

Description:
{result['seo'].get('description', '')}

Alt Text:
{result['seo'].get('alt_text', '')}

Tags:
{', '.join(result['seo'].get('tags', []))}

Keywords:
{', '.join(result['seo'].get('keywords', []))}

Search Terms:
{result['seo'].get('search_terms', '')}
"""
                                zip_file.writestr(f"{result['id']}_seo.txt", seo_text.encode('utf-8'))
                    ...
                    # Now show download button with pre-created ZIP
                    st.download_button(
                        "⬇️ Download ZIP File",
                        data=zip_data,
